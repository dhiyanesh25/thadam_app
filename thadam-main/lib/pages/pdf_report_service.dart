...
// pdf_report_service.dart
// FIXES:
//  • Organisation name shown BOLD and CENTERED on the cover page,
//    between the "Student Progress Report" title and the generated date.
//  • Compact page headers also show org name.
//  • schoolName parameter drives both locations.
//
// pubspec.yaml dependencies needed:
//   pdf: ^3.11.0
//   printing: ^5.13.0
    ...
    children: [
      pw.Row(
        children: [
          pw.Container(
            width: 24, height: 24,
            decoration: pw.BoxDecoration(
                color: _ratingBg(rating), borderRadius: pw.BorderRadius.circular(4)),
            alignment: pw.Alignment.center,
            child: pw.Text('$rating',
                style: pw.TextStyle(font: bold, fontSize: 11, color: _ratingFg(rating))),
          ),
          pw.SizedBox(width: 8),
          pw.Expanded(
            child: pw.Text(_ratingLabel(rating),
                style: pw.TextStyle(font: bold, fontSize: 9, color: _ratingFg(rating))),
          ),
        ],
      ),
      pw.SizedBox(height: 5),
      pw.LayoutBuilder(builder: (ctx, constraints) {
        ...
        return pw.Stack(children: [
          pw.Container(width: totalWidth, height: 5,
              decoration: pw.BoxDecoration(color: _C.border,
                  borderRadius: pw.BorderRadius.circular(3))),
          pw.Container(width: filledWidth, height: 5,
              decoration: pw.BoxDecoration(color: _ratingFg(rating),
                  borderRadius: pw.BorderRadius.circular(3))),
        ]);
      }),
    ],
  ...
  return pw.Container(
    ...
    child: pw.Row(crossAxisAlignment: pw.CrossAxisAlignment.center, children: [
      pw.Container(width: 4, height: 16,
          decoration: pw.BoxDecoration(color: _C.teal,
              borderRadius: pw.BorderRadius.circular(2))),
      pw.SizedBox(width: 8),
      pw.Text(text,
          style: pw.TextStyle(font: bold, fontSize: 11, color: _C.navy, letterSpacing: 0.5)),
    ]),
  );
}
...
pw.Widget _infoPill(String label, String value, pw.Font base, pw.Font bold) {
  return pw.Expanded(
    child: pw.Container(
      padding: const pw.EdgeInsets.symmetric(horizontal: 10, vertical: 10),
      decoration: pw.BoxDecoration(color: _C.surface,
          borderRadius: pw.BorderRadius.circular(8),
          border: pw.Border.all(color: _C.border, width: 0.8)),
      child: pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
        pw.Text(label,
            style: pw.TextStyle(font: base, fontSize: 8, color: _C.textSub, letterSpacing: 0.5)),
        pw.SizedBox(height: 4),
        pw.Text(value,
            style: pw.TextStyle(font: bold, fontSize: 12, color: _C.textPri)),
      ]),
...
pw.Widget _th(String t, pw.Font bold,
    {int flex = 1, pw.TextAlign align = pw.TextAlign.left}) {
  return pw.Expanded(flex: flex,
      child: pw.Text(t,
          style: pw.TextStyle(font: bold, fontSize: 9, color: _C.white, letterSpacing: 0.4),
          textAlign: align));
}

pw.Widget _td(String t, pw.Font font,
    {int flex = 1, PdfColor? color,
      pw.TextAlign align = pw.TextAlign.left, int maxLines = 3}) {
  return pw.Expanded(flex: flex,
      child: pw.Text(t, maxLines: maxLines,
          style: pw.TextStyle(font: font, fontSize: 9, color: color ?? _C.textPri),
          textAlign: align));
}
...
class PdfReportService {
  static Future<void> generate({
    ...
    required String age,
    required String phone,
    required String schoolName,   // ← org name, shown bold+centered
    required List<Map<String, dynamic>> allRecords,
  }) async {
    final baseFont    = pw.Font.helvetica();
    final boldFont    = pw.Font.helveticaBold();
      ...
      pw.Page(
        ...
        build: (ctx) => pw.Column(
          ...
          children: [
            ...
            // ── Hero banner ──
            pw.Container(
              height: 220,   // slightly taller to fit org name
              decoration: const pw.BoxDecoration(
                gradient: pw.LinearGradient(
                  colors: [_C.navy, _C.navyMid, _C.navyLight],
                  begin: pw.Alignment.topLeft,
              ...
              child: pw.Stack(
                children: [
                  ...
                  pw.Positioned(top: -40, right: -40,
                      child: pw.Container(width: 160, height: 160,
                          decoration: pw.BoxDecoration(shape: pw.BoxShape.circle,
                              color: _C.teal.shade(0.15)))),
                  pw.Positioned(top: 10, right: 10,
                      child: pw.Container(width: 90, height: 90,
                          decoration: pw.BoxDecoration(shape: pw.BoxShape.circle,
                              color: _C.teal.shade(0.10)))),

                  pw.Padding(
                    padding: const pw.EdgeInsets.fromLTRB(40, 26, 40, 24),
                    child: pw.Column(
                      crossAxisAlignment: pw.CrossAxisAlignment.start,
                      children: [

                        // ── Row: title left | date pill right ──
                        pw.Row(
                          mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
                          crossAxisAlignment: pw.CrossAxisAlignment.start,
                          children: [
                            ...
                            pw.Container(
                              ...
                              decoration: pw.BoxDecoration(color: _C.teal,
                                  borderRadius: pw.BorderRadius.circular(20)),
                              child: pw.Text('Generated: $reportDate',
                        ...
                        // ── ORGANISATION NAME — BOLD, CENTERED ──
                        pw.Container(
                          width: double.infinity,
                          padding: const pw.EdgeInsets.symmetric(
                              horizontal: 14, vertical: 8),
                          decoration: pw.BoxDecoration(
                            color: _C.teal.shade(0.22),
                            borderRadius: pw.BorderRadius.circular(8),
                            border: pw.Border.all(color: _C.accent.shade(0.5), width: 1),
                          ),
                          child: pw.Center(
                            child: pw.Text(
                              schoolName,
                          ...
                          children: [
                            pw.Container(
                              ...
                              decoration: pw.BoxDecoration(
                                  shape: pw.BoxShape.circle,
                                  color: _C.teal,
                                  border: pw.Border.all(color: _C.accent, width: 2.5)),
                              alignment: pw.Alignment.center,
                            ...
                            pw.Container(
                              padding: const pw.EdgeInsets.symmetric(
                                  horizontal: 10, vertical: 4),
                              decoration: pw.BoxDecoration(
                                  border: pw.Border.all(color: _C.red, width: 1),
                                  borderRadius: pw.BorderRadius.circular(4)),
                              child: pw.Text('CONFIDENTIAL',
                                  style: pw.TextStyle(font: boldFont, fontSize: 8,
                                      color: _C.red, letterSpacing: 1)),
                            ),
                          ],
                        ),
                      ],
                  ...
                  children: [
                    ...
                    pw.Container(
                      ...
                      child: pw.Column(children: [
                        ...
                        pw.Container(
                          padding: const pw.EdgeInsets.all(12),
                          decoration: pw.BoxDecoration(color: _C.tealPale,
                              borderRadius: pw.BorderRadius.circular(6),
                              border: pw.Border.all(
                                  color: _C.teal.shade(0.3), width: 0.8)),
                    ...
                    _sectionHeading('OVERVIEW STATISTICS', boldFont),
                    pw.Row(children: [
                      _statCard('${allRecords.length}', 'Total Observations',
                          _C.teal, baseFont, boldFont),
                      pw.SizedBox(width: 10),
                      _statCard('${grouped.keys.length}', 'Support Areas',
                          _C.navyMid, baseFont, boldFont),
                      pw.SizedBox(width: 10),
                      _statCard(
                          '${grouped.values.fold<int>(0, (s, m) => s + m.keys.length)}',
                          'Challenges Tracked', _C.tealDark, baseFont, boldFont),
                      pw.SizedBox(width: 10),
                      _statCard(_latestDate(allRecords), 'Last Updated',
                          _C.navy, baseFont, boldFont),
                    ]),

                    pw.SizedBox(height: 22),

                    _sectionHeading(
                        'CHALLENGE SUMMARY — HIGHEST RATING PER GOAL', boldFont),
                    _tableHeader([
                      _th('Area of Support', boldFont, flex: 3),
                      _th('Challenge / Goal', boldFont, flex: 3),
                      _th('Entries', boldFont, flex: 1, align: pw.TextAlign.center),
                    ...
                    ...() {
                        ...
                        for (final challengeEntry in areaEntry.value.entries) {
                          ...
                          rows.add(pw.Container(
                            ...
                            child: pw.Row(
                              crossAxisAlignment: pw.CrossAxisAlignment.center,
                              children: [
                                _td(areaEntry.key, baseFont, flex: 3, color: _C.textSub),
                                _td(challengeEntry.key, boldFont, flex: 3),
                                _td('${challengeEntry.value.length}', boldFont,
                                    flex: 1, color: _C.teal, align: pw.TextAlign.center),
                                pw.Expanded(flex: 3,
                                  child: pw.Container(
                                    padding: const pw.EdgeInsets.symmetric(
                                        horizontal: 8, vertical: 4),
                                        ...
                                        pw.Container(width: 18, height: 18,
                                            ...
                                            child: pw.Text('$highest',
                                                style: pw.TextStyle(font: boldFont,
                                                    fontSize: 9, color: _C.white))),
                                        pw.SizedBox(width: 6),
                                        pw.Flexible(child: pw.Text(
                                            _ratingLabel(highest),
                                            style: pw.TextStyle(font: boldFont,
                                                fontSize: 8, color: _ratingFg(highest)))),
                                      ],
                                    ),
                                  ),
                                ),
                      ...
                      rows.add(pw.Container(height: 0.8, color: _C.border));
                      return rows;
                    }(),
                  ],
                ),
            ...
            _pageFooter(studentName, schoolName, '1',
                '${grouped.keys.length + 1}', baseFont, boldFont),
          ],
        ),
      ),
    );
    ...
    for (final areaEntry in grouped.entries) {
      ...
      final allAreaEntries = challenges.values.expand((e) => e).toList();
      allAreaEntries.sort((a, b) => (a['date'] ?? '').compareTo(b['date'] ?? ''));
          ...
          build: (ctx) => pw.Column(
            ...
            children: [
              ...
              pw.Container(
                height: 80,
                color: _C.navy,
                padding: const pw.EdgeInsets.fromLTRB(40, 12, 40, 10),
                child: pw.Column(
                  crossAxisAlignment: pw.CrossAxisAlignment.start,
                  mainAxisAlignment: pw.MainAxisAlignment.center,
                  children: [
                            ...
                            children: [
                              pw.Text(studentName,
                                  style: pw.TextStyle(font: boldFont, fontSize: 14,
                                      color: _C.white)),
                              pw.SizedBox(height: 3),
                              // Org name bold in compact header
                              pw.Text(schoolName,
                  ...
                  child: pw.Column(
                    ...
                    children: [
                      ...
                      pw.Container(
                        ...
                        decoration: pw.BoxDecoration(color: _C.tealPale,
                            borderRadius: pw.BorderRadius.circular(8),
                            border: pw.Border.all(color: _C.teal.shade(0.3), width: 0.8)),
                        child: pw.Row(children: [
                          ...
                          pw.Text(
                              '${challenges.keys.length} challenge${challenges.keys.length == 1 ? '' : 's'}'
                                  '  -  ${allAreaEntries.length} observation${allAreaEntries.length == 1 ? '' : 's'}',
                      ...
                      ...challenges.entries.map((challengeEntry) {
                        ...
                        final entries       = challengeEntry.value;
                        final latest        = entries.last;
                        final latestRating  = (latest['rating'] as int?) ?? 0;
                        final highestRating = entries
                            .map((r) => (r['rating'] as int?) ?? 0)
                            .reduce((a, b) => a > b ? a : b);
                          ...
                          children: [
                            pw.Container(
                              ...
                              child: pw.Row(
                                crossAxisAlignment: pw.CrossAxisAlignment.center,
                                children: [
                                  pw.Expanded(child: pw.Text(challenge,
                            ...
                            pw.Container(
                              ...
                              child: pw.Row(children: [
                                pw.Expanded(child: _ratingBar(latestRating, boldFont, baseFont)),
                                pw.SizedBox(width: 20),
                              ...
                              child: pw.Row(children: [
                                ...
                                pw.SizedBox(width: 86,
                                    child: pw.Text('RATING',
                                        style: pw.TextStyle(font: boldFont, fontSize: 8,
                                            color: _C.accent, letterSpacing: 0.5))),
                                pw.Expanded(child: pw.Text('TEACHER NOTE',
                                    style: pw.TextStyle(font: boldFont, fontSize: 8,
                                        color: _C.accent, letterSpacing: 0.5))),
                            ...
                            ...entries.reversed.toList().asMap().entries.map((e) {
                              ...
                              return pw.Container(
                                decoration: pw.BoxDecoration(
                                  color: isFirst ? _C.greenLight
                                      : isEven ? _C.surface : _C.white,
                                ...
                                child: pw.Row(
                                  ...
                                  children: [
                                        ...
                                        children: [
                                          ...
                                          if (isFirst)
                                            pw.Container(
                                              ...
                                              decoration: pw.BoxDecoration(
                                                  color: _C.green.shade(0.18),
                                                  borderRadius:
                                                  pw.BorderRadius.circular(3)),
                                              child: pw.Text('LATEST',
                                                  style: pw.TextStyle(font: boldFont,
                                                      fontSize: 6, color: _C.green,
                                                      letterSpacing: 0.5)),
                                            ),
                                        ],
                                      ),
                                        ...
                                        child: pw.Row(
                                          ...
                                          children: [
                                            pw.Container(width: 16, height: 16,
                                                ...
                                                child: pw.Text('$r',
                                                    style: pw.TextStyle(font: boldFont,
                                                        fontSize: 8, color: _C.white))),
                                            pw.SizedBox(width: 5),
                                            pw.Flexible(child: pw.Text(_ratingLabel(r),
                                                style: pw.TextStyle(font: boldFont,
                                                    fontSize: 7, color: _ratingFg(r)))),
                                          ],
                                        ),
                                      ),
                                    ),
                                    pw.Expanded(child: pw.Text(note,
                                        style: pw.TextStyle(font: baseFont, fontSize: 9,
                                            color: _C.textSub, lineSpacing: 1.5),
                                        maxLines: 3)),
                                    pw.SizedBox(width: 22,
                                      child: ai
                      ...
                      pw.Container(
                        ...
                        child: pw.Column(
                          ...
                          children: [
                            pw.Text('RATING SCALE',
                                style: pw.TextStyle(font: boldFont, fontSize: 8,
                                    color: _C.textSub, letterSpacing: 0.6)),
                            pw.SizedBox(height: 5),
                            pw.Row(children: [
                              for (int r = 1; r <= 7; r++) ...[
                                pw.Expanded(child: pw.Container(
                                  padding: const pw.EdgeInsets.symmetric(
                                      horizontal: 4, vertical: 4),
                                  decoration: pw.BoxDecoration(color: _ratingBg(r),
                                      borderRadius: pw.BorderRadius.circular(4)),
                                  child: pw.Column(children: [
                                    pw.Text('$r',
                                        style: pw.TextStyle(font: boldFont, fontSize: 9,
              ...
              _pageFooter(studentName, schoolName, '$pageNum', '$totalPages',
                  baseFont, boldFont),
            ],
          ),
        ),
    ...
    await Printing.sharePdf(
      bytes: await doc.save(),
      filename: '${studentName.replaceAll(' ', '_')}_Progress_Report.pdf',
    );
  }
  ...
  static pw.Widget _statCard(String value, String label, PdfColor accent,
    ...
    return pw.Expanded(
      child: pw.Container(
        ...
        child: pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
          pw.Text(value,
              style: pw.TextStyle(font: bold, fontSize: 20, color: _C.white)),
          pw.SizedBox(height: 3),
          pw.Text(label,
              style: pw.TextStyle(font: base, fontSize: 8,
                  color: _C.white.shade(0.8), letterSpacing: 0.3)),
        ]),
      ),
    );
  }

  // ── Footer now includes org name on the left ──
  static pw.Widget _pageFooter(
      String studentName, String orgName, String current,
      String total, pw.Font base, pw.Font bold) {
    return pw.Container(
      height: 36,
      decoration: const pw.BoxDecoration(
          gradient: pw.LinearGradient(colors: [_C.navy, _C.navyMid],
              begin: pw.Alignment.centerLeft,
              end: pw.Alignment.centerRight)),
      padding: const pw.EdgeInsets.symmetric(horizontal: 40),
      child: pw.Row(
          ...
          pw.Row(children: [
            pw.Container(width: 6, height: 6,
                decoration: const pw.BoxDecoration(
                    color: _C.teal, shape: pw.BoxShape.circle)),
            pw.SizedBox(width: 8),
            pw.Text('$studentName  |  $orgName  |  Progress Report',
                style: pw.TextStyle(font: base, fontSize: 8, color: _C.textMuted)),
          ]),
          pw.Text('Page $current of $total',
              style: pw.TextStyle(font: bold, fontSize: 8, color: _C.textMuted)),
        ],
      ),
    );
  }
